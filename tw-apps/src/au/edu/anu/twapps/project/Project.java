...
//	private static DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd:HH:mm:ss:SSS");
	 ...
	 *         conventions.
	 * 
	 * @throws TwAppsException if a project is open | there are no valid characters
	 *                         | the directory cannot be made | directory already
	 *                         exists (if made within nano second i.e. within a
	 *                         loop).
	 */
	public static String create(String name) throws TwAppsException {
		...
		if (name.equals(""))
			throw new TwAppsException("Name does not contain any valid characters (" + givenName + ")");

		try {
			File f = new File(name);
			f.getCanonicalFile();
		} catch (Exception e) {
			throw new TwAppsException(name + "is not a valid file name on this system", e);
		}
		...
		if (projectDirectory.exists()) {
			...
			close();
			throw new TwAppsException("Project directory already exists: " + f.getAbsolutePath());
		}

		if (!projectDirectory.mkdirs())
			throw new TwAppsException("Unable to create project directory: " + projectDirectory.getAbsolutePath());
		return name;
	}
	 ...
	 * Closes the project
	 * 
	 * @throws TwAppsException if not open
	 */
	public static void close() throws TwAppsException {
		if (!isOpen())
			throw new TwAppsException("Project is closed.");
		projectDirectory = null;
	}

	/**
	 * @param directory full path of 3Worlds project directory
	 * @throws TwAppsException when a project is already open | the path is not a
	 *                         valid 3Worlds project path.
	 */
	public static void open(File directory) throws TwAppsException {
		if (isOpen())
			throw new TwAppsException(
					"Cannot open an already open project (" + projectDirectory.getAbsolutePath() + ".");
		if (!isValidProjectFile(directory))
			throw new TwAppsException("Invalid project directory name (" + directory.getName() + ".");
		projectDirectory = directory;
	}

	/**
	 * @return projectDirectory
	 * @throws TwAppsException if Project is closed.
	 */
	public static File getProjectFile() throws TwAppsException {
		if (!isOpen())
			throw new TwAppsException("Project is closed.");
		return projectDirectory;
	}
	 ...
	 * @throws TwAppsException if Project is closed
	 */
	public static String getProjectDirectory() throws TwAppsException {
	 ...
	 * @throws TwAppsException if Project is closed
	 */
	public static String getProjectName() throws TwAppsException {
		if (!isOpen())
			throw new TwAppsException("Project is closed.");
	 ...
	 * @throws TwAppsException if the directory string will not parse correctly
	 */
	public static String extractDisplayName(File directory) throws TwAppsException {
		String[] items = parseProjectName(directory);
		return items[1] + "(" + items[2] + ")";
	}

	public static String getDisplayName() throws TwAppsException {
	 ...
	 * @throws TwAppsException if any of the directories are invalid.
	 */
	public static String[] extractDisplayNames(File[] directories) throws TwAppsException {
	 ...
	 * @throws TwAppsException if not a valid 3Worlds directory
	 */
	public static String extractDateTime(File directory) throws TwAppsException {
		String[] items = parseProjectName(directory);
		return items[2];
	}
	 ...
	 * @throws TwAppsException if project is not open
	 */
	public static String getProjectDateTime() throws TwAppsException {
		if (!isOpen())
			throw new TwAppsException("Project is closed.");
	 ...
	 * @throws TwAppsException if project is not open
	 */
	public static File makeFile(String... pathElements) throws TwAppsException {
		if (!isOpen())
			throw new TwAppsException("Cannot make file from a closed Project");
	...
	private static String[] parseProjectName(File file) throws TwAppsException {
		String name = file.getName();
		String[] items = name.split(sep);
		if (!(items.length == 3))
			throw new TwAppsException(name + " is not a project name. Must have Project_<name>_<dateTime>");
		if (!items[0].equals(PROJECT_DIR_PREFIX))
			throw new TwAppsException(name + " is not a project name. Must start with key work 'Project'");
		try {
			LocalDateTime.parse(items[2], formatter);
		} catch (Exception e) {
			throw new TwAppsException(name + " is not a project name. Must contain valid Date");
		}
		if (!WordUtils.uncapitalize(items[1]).equals(items[1]))
			throw new TwAppsException("First character of " + items[1] + " must be lower case");
